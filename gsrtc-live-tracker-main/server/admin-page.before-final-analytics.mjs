 ...
 *
 * Everything it renders is an aggregate. There is deliberately no "users" panel â€” the app
...
const SHELL_CSS = `
  ...
  * { box-sizing: border-box; }
  ...
  /* Charts. Given real height and a labelled axis â€” a 40px sparkline with no scale is
...
<title>ST Tracker â€” admin</title>
...
<div class="app">
  ...
  <main class="main">
    <header><h1 id="title">Overview</h1><span class="muted" id="stamp">loadingâ€¦</span></header>
    <div id="view"></div>
  </main>
</div>
<script nonce="${nonce}">
 ...
 * a labelled date axis â€” a sparkline without a scale is decoration, not a reading.
 */
function line(rows, pick, label) {
  ...
  if (vals.length < 2) {
    return '<div class="chart-head"><span>' + esc(label) + '</span><b>' + (vals[0] || 0) + '</b></div>'
      + '<div class="empty">One day so far â€” a trend needs at least two.</div>';
  }
  ...
  return '<div class="chart-wrap">'
    + '<div class="chart-head"><span>' + esc(label) + ' Â· ' + total + ' total</span>'
    + '<b>peak ' + max + '</b></div>'
...
function journeys(rows) {
  ...
  return rows.map((s) => {
    ...
    const path = (s.path || []).map((e) =>
      ...
      .join('<span class="arr">â€º</span>');
    return '<div class="jrn"><div class="jrn-head">'
      + '<span class="tag">' + esc(s.device) + '</span>'
      + '<span class="muted">' + esc(when) + ' Â· ' + s.durationSec + 's Â· '
      + esc(s.platform || '?') + (s.standalone ? ' Â· installed' : '') + '</span></div>'
...
async function load() {
  ...
  document.getElementById('stamp').textContent =
    ...
    + ' Â· up ' + Math.floor(d.server.uptimeSec / 3600) + 'h Â· v' + d.server.version;
  ...
  const VIEWS = {
    overview: () => grid([
      card('People', big(p.devices || 0,
        (p.sessions || 0) + ' visits Â· ' + (p.avgSessionSec || 0) + 's average Â· '
        + (p.bouncedPct || 0) + '% did nothing')),
      card('Coming back', big((ret.returningPct || 0) + '%',
        (ret.returning || 0) + ' of ' + (ret.total || 0) + ' had used it before')),
      card('Requests', big(
        d.traffic.requests,
        (d.traffic.lastMinute || 0) + ' in last minute · ' +
        (d.traffic.lastFiveMinutes || 0) + ' in last 5 minutes'
      )),
    ...
    behaviour: () => grid([
      ...
      ['Event', 'Count', 'Devices'],
      events.map((r) => [r.key, r.n, r.devices != null ? r.devices : 'â€”']),
      'events'))),
    ...
    buses: () => grid([
      ...
      'bushist')))
      + wide(card('Regulars â€” devices that keep returning to one bus', table(
    ...
    community: () => grid([
      ...
      card('Replacements reported', (d.community.replacements || []).length
        ? d.community.replacements.map((r) =>
          '<div class="row"><span class="name">' + esc(r.plate) + ' â†’ ' + esc(r.replacement)
    ...
    health: () => grid([
      card('GSRTC API health', (d.uptime || []).length
        ? (d.uptime).map((u) =>
          ...
          + (u.dataErrors ? ' Â· <span style="color:var(--bad)">' + u.dataErrors + ' bad</span>' : '')
          + (u.medianMs != null ? ' Â· ' + u.medianMs + 'ms' : '') + '</b></div>').join('')
 ...
 * header and filtering is live over the rows already rendered â€” no refetch, because the data is
...
function table(headers, rows, id) {
  ...
  return '<input class="filter" data-filter="' + id + '" placeholder="Searchâ€¦">'
