...
/* ST Tracker premium screen transition */
(() => {
  const overlay = document.getElementById('st-page-transition');
  if (!overlay) return;
  ...
  document.addEventListener('click', (event) => {
    ...
    const hasNavigationIntent =
      target.matches(
        '[data-screen], [data-route], .tabbar button, a[href^="#"]'
      );

    if (hasNavigationIntent) transition();
  }, true);
})();
